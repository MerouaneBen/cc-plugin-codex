...
5. **Linked-worktree isolation** — branch reviews launched from Codex worktrees use a temporary local clone under plugin runtime data, then dissociate its objects from the source. Isolation neither writes registration metadata into the common `.git` directory nor depends on source objects surviving for the review's lifetime.
