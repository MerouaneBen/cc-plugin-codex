...
- Resolve permissions from explicit host context before using `exec_command`. If host network is enabled, omit `sandbox_permissions`, including under approval policy `Never`. If network is unavailable and escalation is permitted, use `sandbox_permissions: "require_escalated"` with the justification `Allow the Claude Code companion to contact the Claude API for this requested task.` If network is unavailable and approval policy is `Never`, do not invoke the companion; foreground returns `network-unavailable-no-escalation`, while background reserves the job then runs `background-launch-abort --reason network-unavailable-no-escalation` without spawning a child. Treat unstated network availability as unavailable. Never probe the policy with a deliberately rejected escalation request.
