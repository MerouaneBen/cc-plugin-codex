...
- If host network access is unavailable and the approval policy is `Never`, do not invoke or spawn the companion. For foreground review, surface `network-unavailable-no-escalation`. For background review, reserve the tracked job and immediately run `background-launch-abort --job-id <reserved-job-id> --reason network-unavailable-no-escalation --cwd <workspace-root> --json`; do not spawn a forwarding child.
